...
parser.add_argument('--start_mma_epoch', default=150, type=int,
                    help="manual epoch number (useful on mma)")
parser.add_argument('--start_whole_epoch', default=300, type=int,
                    help="manual epoch number (useful on whole)")
...
parser.add_argument('--stepsize', default=[51, 101, 151, 200, 250, 300, 350, 400], nargs='+', type=int,
                    help="stepsize to decay learning rate")
# parser.add_argument('--stepsize', default=[10, 20, 30, 40, 50, 60, 70, 80], nargs='+', type=int,
                    # help="stepsize to decay learning rate")
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='resnet50tp', help="resnet503d, resnet50tp, resnet50ta, resnetrnn,MMAGGA")
...
def main():
    ...
    # args.start_epoch = 0
    # args.start_mma_epoch = 10
    # args.start_whole_epoch = 20
    # args.stepsize = [6, 11, 16, 21, 26, 31]
    ...
    criterion['center'] = CenterLoss(num_classes=train_dataset.num_train_pids, feat_dim=4096, use_gpu=True)
...
def train(epoch, model, criterion, optimizer, scheduler, trainloader, use_gpu):
    ...
    for batch_idx, sample in enumerate(trainloader):
        ...
        xent_loss = criterion['xent'](outputs[0], pids)+criterion['xent'](outputs[1], pids)+criterion['xent'](outputs[2], pids)+criterion['xent'](outputs[3], pids)
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Loss_htri {loss_htri.val:.4f} Loss_xent {loss_xent.avg:.4f}\t Loss_cnet {loss_cent.avg:.4f}\t'
                  'lr {lr:.10f}\t'
                  .format(
